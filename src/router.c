...
RouterFunction router_find(Router* routers, const char* methodStr, const char* path) {
    ...
    for (int i = 0; !routers[i].end; ++i) {
        ...
        if (routers[i].routers != NULL && strncmp(path, routers[i].path, strlen(routers[i].path)) == 0) {
            RouterFunction nestedFunc = router_find(routers[i].routers, methodStr, path + strlen(routers[i].path));
            if (nestedFunc != NULL) {
                return nestedFunc;
            }
        }
